...
instructions.getRange("A4:G9").values = [
  [1, "修改航点", "打开“航点数据”，修改坐标、需求量或是否启用。", "可以修改", "不要修改", "使用 .xlsx 格式", "按编号读取"],
  [2, "修改限制", "打开“任务参数”，修改容量、航程、无人机数量等。", "可以修改", "说明文字", "不要改工作表名称", "空白数量表示不限"],
  [3, "修改算法", "普通用户可保留默认值；需要实验时再改“算法参数”。", "可以修改", "参数说明", "保存后生效", "只读取所选算法"],
  [4, "保存文件", "按 Ctrl+S 保存，然后关闭 Excel/WPS，避免文件被占用。", "—", "—", "不要另存为 CSV", "读取当前值"],
  [5, "启动程序", "双击“启动程序.bat”，按中文提示选择这个文件。", "—", "—", "原文件不会被改写", "先校验再规划"],
  [6, "查看结果", "结果目录中会生成路线图、收敛图、JSON 和 CSV。", "—", "—", "可重复运行", "固定种子可复现"],
];
...
for (const [row, value] of [
  ...
  [12, "欧氏距离适合普通平面坐标；经纬度距离要求第一列为纬度、第二列为经度。"],
  [13, "本项目不需要 YOLO，也不会控制真实无人机飞行。"],
]) {
  instructions.getRange(`A${row}:G${row}`).merge();
  instructions.getRange(`A${row}`).values = [[value]];
}
instructions.getRange("A11:G13").format = {
  fill: COLORS.green,
  font: { bold: true, color: COLORS.navy },
  wrapText: true,
...
title(waypoints.getRange("A1:G1"), "航点数据（黄色区域可修改）");
waypoints.getRange("A3:G3").values = [[
  "编号", "名称", "X或纬度", "Y或经度", "需求量", "是否启用", "备注"
]];
sectionHeader(waypoints.getRange("A3:G3"));
...
waypoints.getRange(`A4:G${3 + waypointRows.length}`).values = waypointRows;
editable(waypoints.getRange("A4:G103"));
waypoints.getRange("A4:A103").format.numberFormat = "0";
waypoints.getRange("C4:E103").format.numberFormat = "0.00";
waypoints.getRange("A4:A103").dataValidation = {
  rule: { type: "whole", operator: "between", formula1: 0, formula2: 99999 },
};
waypoints.getRange("E4:E103").dataValidation = {
...
waypoints.getRange("E4:E103").conditionalFormats.add("cellIs", {
  operator: "lessThan",
  formula: 0,
  format: { fill: COLORS.red, font: { color: "#9C0006" } },
});
const waypointTable = waypoints.tables.add(`A3:G${3 + waypointRows.length}`, true, "WaypointsTable");
...
waypoints.getRange("J3").formulas = [['=COUNTIF(F4:F103,"是")']];
waypoints.getRange("J4").formulas = [['=SUMIF(F4:F103,"是",E4:E103)']];
...
waypoints.getRange("C:E").format.columnWidth = 15;
waypoints.getRange("F:F").format.columnWidth = 12;
waypoints.getRange("G:G").format.columnWidth = 32;
...
taskParams.getRange("A4:D11").values = [
  ["问题类型", "CDVRP", "TSP=单机；CDVRP=无人机群", "是"],
  ["算法", "ACO", "可选 ACO、GA、HPSO、SA", "是"],
  ["距离模式", "欧氏距离", "平面坐标选欧氏；真实 GPS 选经纬度", "是"],
  ["距离单位", "km", "仅用于结果显示；经纬度模式固定为 km", "是"],
  ["单机容量", 20, "CDVRP 中一架无人机最多承载的需求量", "CDVRP"],
  ["单机最大航程", 500, "每条闭环航线允许的最大距离", "CDVRP"],
  ["最大无人机数量", null, "留空表示不限制", "否"],
  ["随机种子", 42, "相同数据和种子可得到相同结果", "是"],
];
taskParams.getRange("A4:A11").format = {
  fill: COLORS.gray,
  font: { bold: true, color: COLORS.darkGray },
  borders: { preset: "all", style: "thin", color: COLORS.border },
};
editable(taskParams.getRange("B4:B11"));
explanatory(taskParams.getRange("C4:D11"));
taskParams.getRange("C4:D11").format.borders = { preset: "all", style: "thin", color: COLORS.border };
...
taskParams.getRange("B6").dataValidation = { rule: { type: "list", values: ["欧氏距离", "经纬度距离"] } };
taskParams.getRange("B8:B9").dataValidation = {
  rule: { type: "decimal", operator: "greaterThan", formula1: 0 },
};
taskParams.getRange("B10").dataValidation = {
...
taskParams.getRange("4:11").format.rowHeight = 28;
...
for (const sheetName of ["使用说明", "航点数据", "任务参数", "算法参数"]) {
