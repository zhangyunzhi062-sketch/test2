...
这是一个由 Excel/WPS 表格驱动的二维/三维任务级路径规划程序，适合课程设计、算法学习和小规模实验。
...
- 概述：在原有 TSP/CDVRP 与 ACO、GA、HPSO、SA 基础上增加三维静态环境规划。
- 目标：每个任务点只访问一次，满足容量、航程和无人机数量限制，同时保证每段实际飞行折线不穿越 Excel 中启用的障碍物。
- 步骤：读取三维航点和长方体禁区 → 转换本地或经纬度坐标 → 建立三维可见图并计算任务点间避障距离 → 交给原有八个求解入口优化访问顺序 → 展开并输出完整三维路径。
...
项目兼容原二维中文模板和 MATLAB `City`、`Demand`、`Capacity`、`Travelcon` 格式。固定相同数据、算法参数和随机种子时，结果可复现。
